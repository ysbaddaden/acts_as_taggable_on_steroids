...
class Tag < ActiveRecord::Base
  ...
  class << self
    ...
    def counts(options = {})
      ...
      tags = joins(:taggings)
      tags = tags.having(["COUNT(#{quoted_table_name}) >= ?", options[:at_least]]) if options[:at_least]
      tags = tags.having(["COUNT(#{quoted_table_name}) <= ?", options[:at_most]])  if options[:at_most]
